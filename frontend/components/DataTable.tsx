...
export function DataTable({ rows, columns, detailBase, filterField, onExport }: Props) {
  ...
  const pageSize = 12;
  const indexedRows = useMemo(() => rows.map((row, index) => ({
    row,
    index,
    filterValue: filterField ? displayValue(row[filterField]) : "",
    searchText: Object.values(row).map((value) => displayValue(value).toLowerCase()).join("\n"),
  })), [filterField, rows]);
  const filterOptions = useMemo(() => filterField ? Array.from(new Set(indexedRows.map((entry) => entry.filterValue).filter((value) => value !== "-"))).sort() : [], [filterField, indexedRows]);
  const normalizedQuery = deferredQuery.trim().toLowerCase();
  const filtered = useMemo(() => {
    const next = indexedRows.filter((entry) => {
      if (filterField && filterValue && entry.filterValue !== filterValue) return false;
      return !normalizedQuery || entry.searchText.includes(normalizedQuery);
    });
    next.sort((a, b) => displayValue(a.row[sortKey]).localeCompare(displayValue(b.row[sortKey]), undefined, { numeric: true }));
    return next;
  }, [filterField, filterValue, indexedRows, normalizedQuery, sortKey]);
  const pageCount = Math.max(1, Math.ceil(filtered.length / pageSize));
  const visible = filtered.slice(page * pageSize, page * pageSize + pageSize);
  const hasOpenColumn = useMemo(() => Boolean(detailBase) || rows.some((row) => typeof row.open_href === "string" && row.open_href), [detailBase, rows]);
  useEffect(() => {
  ...
  return (
    <div className="panel">
      ...
      <div className="table-wrap"><table><thead><tr>{columns.map((column) => <th key={column} onClick={() => setSortKey(column)}>{formatLabel(column)}</th>)}{hasOpenColumn ? <th>Open</th> : null}</tr></thead><tbody>{visible.map(({ row, index }) => {
        ...
        return <tr key={String(row.id ?? row.open_href ?? index)}>{columns.map((column) => <td key={column}>{renderCell(column, row[column])}</td>)}{hasOpenColumn ? <td>{href ? <Link className="icon-button" title={title} href={href}><Eye size={16} /></Link> : null}</td> : null}</tr>;
      })}</tbody></table></div>

...
function detailRecordForSelection(selection: StreetMapSelection): Record<string, unknown> {
  ...
  if (selection.kind === "opgw_cable") {
    return {
      ...
      sourceRouteCableId: selection.record.properties.id,
      ...
      cableIdBoundary: "Route source only. Planning cable IDs are splice-to-splice cable sections.",
  ...
  if (selection.kind === "opgw_cable_section") {
    return {
      ...selection.record.properties,
      cableBoundary: `${selection.record.properties.fromSplicePointId} to ${selection.record.properties.toSplicePointId}`,
      cableIdRule: "Cable IDs are assigned only to the cable span between exactly two splice points.",

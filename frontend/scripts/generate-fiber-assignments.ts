...
import { FIBER_ASSIGNMENTS_PATH, FIBER_SPLICES_PATH, PATCH_PANELS_PATH, STRANDS_PATH, chooseWeighted, createSeededRandom, readJson, readOpgwCables, readSpliceClosures, readStructures, round, writeJson } from "./fiber-network-utils";
...
async function main() {
  ...
  cables.features.forEach((cable, cableIndex) => {
    const assignmentCount = cable.properties.fiberCount >= 72 ? 3 : 2;
    const strandWidth = cable.properties.fiberCount >= 144 ? 8 : cable.properties.fiberCount >= 96 ? 6 : cable.properties.fiberCount >= 48 ? 4 : 2;
    for (let attempt = 0; attempt < assignmentCount; attempt += 1) {
      const available = (strandsByCable.get(cable.properties.id) || []).filter((strand) => !strand.assignmentId && !["assigned", "reserved", "faulted", "retired"].includes(strand.status));
      ...
      const spliceIds = chooseSplices(spliceByCable.get(cable.properties.id) || [], selected.map((strand) => strand.strandNumber));
      ...
      const assignment: FiberAssignment = {
        id: assignmentId,
        assignmentName: `${serviceType}-${cable.properties.id}-${String(attempt + 1).padStart(2, "0")}`,
        synthetic: true,
        serviceType,
        status: assignmentStatus,
        aEndStructureId: cable.properties.startStructureId,
        zEndStructureId: cable.properties.endStructureId,
        cableIds: [cable.properties.id],
        strandSegments: [{
          cableId: cable.properties.id,
          strandNumbers: selected.map((strand) => strand.strandNumber),
          fromStructureId: cable.properties.startStructureId,
          toStructureId: cable.properties.endStructureId,
        }],
        spliceIds,
        estimatedDistanceMiles: cable.properties.routeMiles,
        estimatedLossDb: round(cable.properties.routeMiles * 0.25 + spliceLoss + 1, 3),
      ...
      patchPanels.forEach((panel) => {
        if (!panel.fiberCableIds.includes(cable.properties.id)) return;
